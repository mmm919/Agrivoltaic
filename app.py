...
def top_bar(title_text: str, subtitle_text: str, pills: List[str], accent_index: Optional[int] = 0):
    pills_html = ""
...
<div class="topbar">
  <div>
    <div class="topbar-title">{title_text}</div>
    <div class="topbar-sub">{subtitle_text}</div>
  </div>
  <div class="topbar-right">{pills_html}</div>
</div>
""",
        unsafe_allow_html=True,
    )
...
def kpi_circles(pv: float, comfort: float, water: float):
    st.markdown(
        f"""
<div class="kpi-row">
  <div class="kpi {kpi_class(pv)}"><div class="v">{int(round(pv))}%</div><div class="l">PV performance</div></div>
  <div class="kpi {kpi_class(comfort)}"><div class="v">{int(round(comfort))}%</div><div class="l">Crop comfort</div></div>
  <div class="kpi {kpi_class(water)}"><div class="v">{int(round(water))}%</div><div class="l">Water savings</div></div>
</div>
""",
        unsafe_allow_html=True,
    )
...
def farm_conditions_card(location: str, solar: str, wind: str, humidity: str):
    st.markdown(
        f"""
<div class="card">
  <div class="section-title">Farm location condition</div>
  <div class="muted">{location}</div>
  <div style="display:flex; gap:22px; margin-top:10px; flex-wrap:wrap;">
    <div><b>Solar</b><div class="small">{solar}</div></div>
    <div><b>Wind</b><div class="small">{wind}</div></div>
    <div><b>Humidity</b><div class="small">{humidity}</div></div>
  </div>
</div>
""",
        unsafe_allow_html=True,
...
def soil_factor(soil: str) -> float:
    if soil == "Dry":
        return 0.0
    if soil == "Medium":
        return 0.5
...
def simulate_scenario(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    leaf_cooling = 25.0 * (0.35 * shade + 0.35 * sf + 0.30 * lai_effect)
    leaf_cooling *= (0.75 + 0.25 * ventilation)
    leaf_cooling = clamp(leaf_cooling, 0.0, 25.0)

    water_savings = 10.0 + 35.0 * (0.40 * shade + 0.40 * sf + 0.20 * lai_effect)
    water_savings = clamp(water_savings, 0.0, 50.0)

    heat_index_reduction = 0.5 * leaf_cooling + 4.0 * sf
    heat_index_reduction = clamp(heat_index_reduction, 0.0, 20.0)

    comfort_score = 45.0 + 2.0 * leaf_cooling + 0.35 * water_savings - 0.6 * abs(canopy_height - 1.4) * 10.0
    comfort_score = clamp(comfort_score, 0.0, 100.0)

    pv_performance = 65.0 + pv_gain
    pv_performance = clamp(pv_performance, 0.0, 100.0)

    water_kpi = clamp(water_savings * 2.0, 0.0, 100.0)
    return {
        "pv_performance": pv_performance,
        "crop_comfort": comfort_score,
        "water_savings_kpi": water_kpi,
        "leaf_cooling_c": leaf_cooling,
        "pv_gain_percent": pv_gain,
        "water_savings_percent": water_savings,
        "heat_index_reduction_c": heat_index_reduction,
        "comfort_score": comfort_score,
    }


def story_curves(result: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
    hours = np.arange(0, 24)
    base = 28 + 10 * np.sin((hours - 6) / 24 * 2 * np.pi)
    cooling = result["leaf_cooling_c"]
    hi_red = result["heat_index_reduction_c"]
    pv_gain = result["pv_gain_percent"]
    ...
    df_leaf = pd.DataFrame({"hour": hours, "leaf_temp_c": leaf_curve})
    df_hi = pd.DataFrame({"hour": hours, "heat_index_proxy": heat_index_curve})
    df_pv = pd.DataFrame({"hour": hours, "pv_benefit_proxy": pv_curve})
    return {"leaf": df_leaf, "hi": df_hi, "pv": df_pv}


def page_home():
    top_bar("Farm Dashboard", "Location, KPIs, alerts, updates", ["Backend", "Home"], 0)
    ...
    if not locations:
        st.error("No locations found. Add one in Settings.")
        return

    names = list(locations.keys())
    if "location" not in st.session_state:
        st.session_state["location"] = default_location
    if st.session_state["location"] not in names:
        st.session_state["location"] = names[0]
    ...
    with left:
        ...
        st.session_state["location"] = st.selectbox(
            "Choose location", names, index=names.index(st.session_state["location"])
        )
        st.markdown("</div>", unsafe_allow_html=True)

        loc = st.session_state["location"]
        cond = locations.get(loc, {})
            ...
            for i, a in enumerate(alerts[:5]):
                ...
                with c1:
                    if t == "Info":
                        ...
                        st.error(f"{m}\n\nAdded {ts}")
                with c2:
                    if st.button("Remove", key=f"home_rm_alert_{i}"):
                        api_delete(f"/alerts/{i}")
                        st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with right:
        ...
        st.markdown(
            '<div class="small" style="margin-top:10px;">These KPIs come from backend settings for now.</div>',
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        with a_col:
            ...
            if ok:
                if a_msg.strip():
                    api_post("/alerts", {"type": a_type, "message": a_msg.strip()})
        ...
        with u_col:
            ...
            if ok2:
                if u_title.strip() and u_body.strip():
                    api_post("/updates", {"title": u_title.strip(), "body": u_body.strip()})
                    st.success("Update added")
                    st.rerun()
                else:
                    st.warning("Update title and details cannot be empty")
        ...
        if not updates:
            ...
            for i, u in enumerate(updates[:5]):
                ...
                with c1:
                    ...
                    st.caption(f"{body}\n\nAdded {ts}")
                with c2:
                    if st.button("Remove", key=f"home_rm_update_{i}"):
                        api_delete(f"/updates/{i}")
                        st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)


def page_design():
    top_bar("Design", "Choose parameters and run simulation", ["Design", "Scenario"], 0)
    ...
    with c2:
        ...
        if open_cropland:
            tracking = st.toggle("Single axis tracking", value=False, disabled=True)
        else:
            tracking = st.toggle("Single axis tracking", value=False)

    st.markdown("</div>", unsafe_allow_html=True)

    params = {
        "panel_height_m": panel_height,
        "panel_spacing_m": panel_spacing,
        "tilt_deg": tilt,
        "canopy_height_m": canopy_height,
        "lai": lai,
        "soil_wetness": soil,
        "single_axis_tracking": tracking,
    }
    ...
    if run:
        ...
        st.session_state["design_result"] = {"params": params, "result": result, "mode": mode}
    ...
    if st.session_state["design_result"]:
        ...
        if st.button("Save scenario"):
            st.session_state["saved_scenarios"].append(
                {"name": scenario_name.strip() or "Scenario", "params": params, "result": out}
            )
...
def page_compare():
    top_bar("Compare", "Compare scenarios and view the story", ["Compare", "Charts"], 0)

    if "saved_scenarios" not in st.session_state:
        st.session_state["saved_scenarios"] = []

    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Compare scenarios</div>', unsafe_allow_html=True)

    left, mid, right = st.columns(3, gap="medium")

    def scenario_card(col, title, result):
        with col:
            st.markdown('<div class="card-soft">', unsafe_allow_html=True)
            st.markdown(f"**{title}**")
            st.caption("Leaf cooling, water savings, PV gain, heat index, comfort")
            st.metric("PV gain (%)", f'{result["pv_gain_percent"]:.1f}')
            st.metric("Leaf cooling (C)", f'{result["leaf_cooling_c"]:.1f}')
            st.metric("Water savings (%)", f'{result["water_savings_percent"]:.1f}')
            st.metric("Comfort score", f'{result["comfort_score"]:.0f}')
            st.markdown("</div>", unsafe_allow_html=True)

    base_open = simulate_scenario(
        {
            "panel_height_m": 0.8,
            "panel_spacing_m": 1.2,
            "tilt_deg": 0.0,
            ...
            "lai": 3.0,
            "soil_wetness": "Medium",
            "single_axis_tracking": False,
        }
    )
    pv_only_dry = simulate_scenario(
        {
            "panel_height_m": 3.0,
            "panel_spacing_m": 4.0,
            "tilt_deg": 25.0,
    ...
    if st.session_state["saved_scenarios"]:
        s = st.session_state["saved_scenarios"][-1]
        agr_title = s["name"]
        agr_res = s["result"]
    else:
        agr_title = "Agrivoltaic"
        agr_res = simulate_scenario(
            {
                "panel_height_m": 2.0,
                "panel_spacing_m": 3.0,
                "tilt_deg": 25.0,
    ...
    scenario_card(left, agr_title, agr_res)
    scenario_card(mid, "Open cropland", base_open)
    scenario_card(right, "PV only dry soil", pv_only_dry)

    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("")
    view_story = st.button("View the story")
    ...
    if view_story:
        ...
        with c1:
            fig1 = px.line(curves["leaf"], x="hour", y="leaf_temp_c",
                           labels={"hour": "Hours of day", "leaf_temp_c": "Leaf temperature (C)"})
            st.plotly_chart(fig1, use_container_width=True)
        with c2:
            fig2 = px.line(curves["hi"], x="hour", y="heat_index_proxy",
                           labels={"hour": "Hours of day", "heat_index_proxy": "Heat index proxy"})
            st.plotly_chart(fig2, use_container_width=True)
        with c3:
            fig3 = px.line(curves["pv"], x="hour", y="pv_benefit_proxy",
                           labels={"hour": "Hours of day", "pv_benefit_proxy": "PV benefit proxy"})
...
def page_report():
    top_bar("Report", "Inspect CSV files", ["Local files", "Report"], 0)
    ...
    if not sensor_files:
        ...
        path = next(p for p in sensor_files if p.name == sname)
        df = pd.read_csv(path)
    ...
    if not pred_files:
        ...
        path2 = next(p for p in pred_files if p.name == pname)
...
def page_settings():
    top_bar("Settings", "Manage locations and KPI values", ["Backend", "Settings"], 0)
    ...
    locations = locs_data.get("locations", {})
    default_location = locs_data.get("default_location", "")
    names = list(locations.keys())
    ...
    if names:
        ...
        if st.button("Save default"):
            api_put(f"/default_location/{new_default}", {})
            st.success("Saved")
            st.rerun()
    else:
    ...
    if save_loc:
        if name.strip():
            api_put(f"/locations/{name.strip()}", {"solar": solar, "wind": wind, "humidity": humidity})
            st.success("Saved")
            st.rerun()
        else:
    ...
    if names:
        ...
        if st.button("Delete selected"):
            api_delete(f"/locations/{del_name}")
            st.success("Deleted")
            st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.button("Save KPI values"):
        api_put("/kpis", {"pv": pv, "comfort": comfort, "water": water})
        st.success("Saved")
    st.markdown("</div>", unsafe_allow_html=True)
